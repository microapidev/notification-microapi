    	...
    	try {
    		...
    		if ( $request->email != "" && $request->recovery_password != "" ) {
				...
				if ( User::where('email', $user->email)->exists() ){
					...
					return response()
			        	->json([ "success" => "false", "message" => "Email already exists" ], 500);
				}
    		...
    		if ( $email != "" && $recovery_password != "") {
    			...
    			if ( User::where('email', $email)->exists() ) {
    				...
    				if ( $user_details ) {
    					...
    					return response()
	            			->json([ "success" => "true", "message" => "User Details Successfully Retrieved", "data" => $user_details ], 200);
    				} else {

	            		return response()
	            			->json([ "success" => "false", "message" => "Incorrect Recovery Password" ], 400);
    				}
    			} else {
